...
def scrape_booking_region(dest_id, checkin_date, checkout_date):
    ...
    try:
        ...
        time.sleep(random.uniform(0.3, 0.5)) # Delay between 2 and 5 seconds
        ...
        for hotel in hotels:
            ...
            try:
                ...
                logging.error(f"Error obteniendo url, id, o localidad: {e}")
                hotel_data['url'] = None
                hotel_data['id'] = None
            ...
            try:
                ...
                if price_text:
                    ...
                    try:
                        ...
                        #print(f"Could not convert price to integer: {cleaned_price_text}")
...
def scraping():
    ...
    dest_ids_to_scrape = ['1363', '755', '766', '747', '774', '758', '750', '759'] #['1363'] # Uncomment this line and comment the one above to scrape only Almería
    ...
    for dest_id in dest_ids_to_scrape:
        ...
        for i in range(15): # Changed to range(3) for 3 days
            ...
            if hotels_data:
                ...
                json_filename = f"{province_name.lower().replace(' ', '_')}_{checkin_date.strftime('%Y%m%d')}.ndjson" # Using .jsonl for line-delimited JSON
       ...
       #print(f"Fin de scraper booking.")
    logging.info("Fin de scraper booking.")

if __name__ == "__main__":
    scraping()
    #Descomentar siguiente bloque en produccion
